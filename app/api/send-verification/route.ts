...
export async function POST(req: NextRequest) {
  try {
    ...
    const templateId = Number(process.env.BREVO_VERIFY_TEMPLATE_ID ?? process.env.BREVO_TEMPLATE_ID_VERIFY);
    if (!apiKey || !Number.isFinite(templateId)) {
      return NextResponse.json({ ok: false, code: "missing-env", error: "Missing Brevo key or verify template id" }, { status: 500 });
    }
    ...
    await axios.post(
      "https://api.brevo.com/v3/smtp/email",
      {
        ...
        params: {
          ...
          logoUrl: process.env.EMAIL_LOGO_URL || `${appUrl}/logo.png`,
        },
      },
      { headers: { "api-key": apiKey, "content-type": "application/json" }, timeout: 15000 }
    );

    return NextResponse.json({ ok: true, uid: user.uid, link: verifyUrl });
  } catch (err: any) {
